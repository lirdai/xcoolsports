...
import { api, selectLatestMessage } from '@xcoolsports/data';

const DeleteMessage = ({ username, onClose }) => {
    const { t, i18n } = useTranslation();
    const isMounted = useRef(true);
    const theme = useContext(ThemeContext);

    const [deleteMessages] = api.endpoints.deleteAllMessagesByUser.useMutation();
    const latestMessage = useSelector(selectLatestMessage);

    const [openDeleteModal, setOpenDeleteModal] = useState(false);

    const handleDeleteMessage = async () => {
        deleteMessages({ partner: username, lte: latestMessage.id });
