...
const Following = ({ navigation }) => {
  ...
  const followings = useSelector((state) => selectManyUsersByUsername(state, followingIds));
  const { data, isLoading, isFetching, isError, error } = api.endpoints.getFollowing.useQuery({ username: currentUser.username, page }, { skip: !hasNext });
  ...
  useEffect(() => {
    if (data?.users) setFollowingIds((prev) => prev.concat(data.users.map((r) => r.username)));
  }, [data?.users]);

  const isEmpty = !isLoading && followings && followings.length === 0;
  ...
  return (
    ...
    >
      ...
      {(!isLoading && !isError && !isEmpty && followings) &&
        <FlatList 
